...
const App = {
  ...
  vocabMap() {
    const map = {};
    this.data.vocabulary?.words?.forEach(w => { map[w.id] = w; });
    return map;
  },
  ...
  renderLessons() {
    ...
    levels.forEach(level => {
      ...
      lessons.forEach(lesson => {
        ...
        html += `
          <div class="card card-hover lesson-card" onclick="App.openLesson('${lesson.id}','${level.id}')">
  ...
  openLesson(lessonId, levelId) {
    const level = this.data.lessons.levels.find(l => l.id === levelId);
    const lesson = level?.lessons.find(l => l.id === lessonId);
    if (!lesson) return;

    this.selectedLesson = lesson;
    const vm = this.vocabMap();
    const vocabHtml = (lesson.vocabIds || []).map(id => {
      const w = vm[id];
      if (!w) return '';
      return `
        <div class="vocab-row">
          ...
          <button class="btn btn-sm btn-outline" onclick="App.playAudio('${w.pinyin}')">🔊</button>
        </div>`;
    }).join('');
    ...
    document.getElementById('lessonDetail').innerHTML = `
      ...
      <div class="flex gap-1">
        ...
        <button class="btn btn-outline" onclick="App.startQuiz('${lesson.id}')">📝 Làm quiz</button>
      </div>`;
  ...
  playAudio(text, rate = 0.85) {
    if (typeof Voice !== 'undefined') {
      if (rate <= 0.6) return Voice.speakSlow(text);
      if (rate >= 1) return Voice.speakNormal(text);
      return Voice.speak(text, { rate });
    }
      ...
      <div class="flashcard-wrap">
        <div class="flashcard" id="fcard" onclick="document.getElementById('fcard').classList.toggle('flipped')">
          ...
          <div class="flashcard-face flashcard-back">
            ...
            <button class="btn btn-sm btn-outline mt-2" onclick="event.stopPropagation();App.playAudio('${w.pinyin}')">🔊 Nghe</button>
          </div>
        </div>
      </div>
      <div class="srs-row">
    ...
    switch (skill) {
      case 'listen':
        body = `<div class="hanzi hanzi-lg text-center mb-2">???</div>
          ...
          <button class="btn btn-primary" style="display:block;margin:0 auto" onclick="App.playAudio('${w.pinyin}')">🔊 Phát âm</button>
  ...
  renderQuiz() {
    ...
    document.getElementById('quizList').innerHTML = `
      ...
      quizzes.map(q => `
      <div class="card card-hover" onclick="App.startQuiz('${q.id}')">
        <div class="card-title">📝 ${q.title}</div>
        <div class="card-desc">${q.questions.length} câu · Giải thích chi tiết</div>
        <button class="btn btn-primary btn-sm mt-2">Làm bài →</button>
      </div>`).join('') ||
  ...
  showQuizQuestion() {
    ...
    if (!q) {
      document.getElementById('quizArea').innerHTML = `
        <div class="card" style="text-align:center;padding:2rem">
          ...
          <button class="btn btn-primary mt-2" onclick="App.renderQuiz()">← Quay lại</button>
        </div>`;
    ...
    document.getElementById('quizArea').innerHTML = `
      <div class="card">
        ...
        ${q.audioText ? `<button class="btn btn-sm btn-outline mb-2" onclick="Voice.speak('${q.audioText}')">🔊 Nghe</button>` : ''}
        <div id="quizOptions">
          ${q.options.map((o, i) => `<button class="quiz-option" onclick="App.answerQuiz(${i})">${o}</button>`).join('')}
        </div>
  ...
  renderDictionary(query = '') {
    ...
    document.getElementById('dictResults').innerHTML = filtered.map(e => `
      <div class="dict-item" onclick="App.playAudio('${e.pinyin}')">

import type { SrsCard, Word } from '../types/data';

const intervals = [1, 3, 7, 14, 30, 60];

export function initCard(wordId: string): SrsCard {
  return { wordId, ease: 2.5, interval: 0, repetitions: 0, nextReview: Date.now(), lastReview: null };
}

export function getCard(wordId: string, srsCards: Record<string, SrsCard>): SrsCard {
  if (!srsCards[wordId]) srsCards[wordId] = initCard(wordId);
  return srsCards[wordId];
}

/** quality: 0=again, 1=hard, 2=good, 3=easy */
export function reviewCard(card: SrsCard, quality: number): SrsCard {
  const now = Date.now();
  card.lastReview = now;
  if (quality === 0) {
    card.repetitions = 0;
    card.interval = 0;
    card.nextReview = now + 60000;
    return card;
  }
  if (quality === 1) {
    card.ease = Math.max(1.3, card.ease - 0.15);
    card.interval = Math.max(1, Math.floor(card.interval * 0.5));
  } else if (quality === 2) {
    card.repetitions += 1;
    card.interval = intervals[Math.min(card.repetitions - 1, intervals.length - 1)];
  } else {
    card.repetitions += 1;
    card.ease += 0.1;
    card.interval = intervals[Math.min(card.repetitions, intervals.length - 1)] * 2;
  }
  card.nextReview = now + card.interval * 24 * 60 * 60 * 1000;
  return card;
}

export function getDueCards(words: Word[], srsCards: Record<string, SrsCard>) {
  const now = Date.now();
  return words.filter(w => {
    const card = srsCards[w.id];
    if (!card) return true;
    return card.nextReview <= now;
  });
}

export function getSrsStats(srsCards: Record<string, SrsCard>) {
  const cards = Object.values(srsCards);
  const now = Date.now();
  return {
    total: cards.length,
    due: cards.filter(c => c.nextReview <= now).length,
    mastered: cards.filter(c => c.repetitions >= 4).length,
  };
}
